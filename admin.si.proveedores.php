...
$codigo='';
	...
	if($opcion=='UPD'){
		...
		$query="SELECT bienvenida,pie,estatus FROM mensaje where id_mensaje='$idreg' limit 1";
			...
			if(!empty($_FILES['filefoto']['name'])){
				...
				if(move_uploaded_file($_FILES['filefoto']['tmp_name'], $target_path)) {
					...
					if($idreg!=0){
						...
						$tranx="insert into mensaje (bienvenida,pie,estatus)  values('$titulo', '$codigo',$publicar);";
				...
				if($idreg!=0){
					//echo $foto=$_POST["hiurl"];
					 ...
					 $tranx="update mensaje set bienvenida='$titulo',pie='$codigo',estatus='$publicar'where id_mensaje=$idreg";
